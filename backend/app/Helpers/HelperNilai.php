...
class HelperNilai extends HelperAkademik {
  /**
  ...
  */
  ...
  public function getKHS ($ta, $idsmt, $cek_isikuesioner=false) {
    ...
    else
    {
      ...
      {
        $data = [
          'idkrsmatkul' => $b->idkrsmatkul,
          'idsmt' => $b->idsmt,
          'tahun' => $b->tahun,
          'kmatkul' => $this->getKMatkul($b->kmatkul),
          'nmatkul' => $b->nmatkul,
          'sks' => $b->sks,
          'n_kual' => $b->n_kual,
          'nidn' => $b->nidn,
          'nama_dosen' => $b->nama_dosen,
          'telah_isi_kuesioner' => $b->telah_isi_kuesioner
        ];        
        ...
        if ($data['telah_isi_kuesioner'])
        {
          $data['keterangan'] = '-';
        }
        else
        {
          $data['keterangan'] = $hm == '-' ? 'BELUM ISI NILAI' : 'BELUM ISI KUESIONER';
        }
        $result[$a] = $data;                    
      }
    }
  ...
  */
  ...
  public function getIPKSampaiTASemester($tahun_sekarang, $semester_sekarang, $mode=null) {
    ...
    '))
    ->where('nim', $nim)
    ->whereRaw("tasmt <= '$ta_smt'")
    ->groupBy('kmatkul')   
    ->orderBy('kmatkul', 'asc');

    $dn = \DB::table('v_nilai_khs AS A')
    ->select(\DB::raw('
      n_kual,
      sks
    '))
    ->joinSub($subquery, 'B', function($join) {
      $join->on('A.idkrsmatkul', '=', 'B.idkrsmatkul');
    }) 
    ->get();

    $this->DataNilai = json_decode(json_encode($dn), true);
  ...
  */
  ...
  public function getIPS() {				
    ...
    if ($countM > 0 && $totalSks > 0)
    {
      $ips=@ bcdiv($countM, $totalSks, 2);
      return $ips;
    }
    else
    {

<template>
  <DMasterLayout :showrightsidebar="false">
    ...
    <template v-slot:page-content>
      <b-form @submit.prevent="onSubmit" name="frmdata" id="frmdata" v-if="$store.getters['auth/can']('DMASTER-DOSEN-KATEGORI-KEGIATAN_UPDATE')">
        <b-card
          no-body
          class="card-primary card-outline"
        >
          <template #header>
            <h3 class="card-title">Ubah Kategori Kegiatan</h3>
...
<script>
  ...
  export default {
    name: 'KategoriKegiatanDosenEdit',
    created() {
      this.idkategori =this.$route.params.idkategori
      this.initialize()
    },
    setup() {
      return {
    ...
    methods: {
      ...
      async initialize() {
        var url = '/dmaster/dosen/kategorikegiatan/' + this.idkategori;
      ...
      async onSubmit() {
        if (!this.v$.formdata.$invalid) {
          ...
          this.$ajax.post('/dmaster/dosen/kategorikegiatan/' + this.idkategori,
            {
              _method: 'PUT',

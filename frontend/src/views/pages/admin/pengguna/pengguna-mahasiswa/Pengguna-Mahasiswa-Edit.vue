<template>
  <PenggunaSistemLayout>
    ...
    <template v-slot:page-content>
      <b-container
        fluid
        v-if="$store.getters['auth/can']('SYSTEM-USERS-MAHASISWA_UPDATE')"
      >
        <b-row>
          <b-col>
            <EditForm
              urlfront="/sistem-pengguna/mahasiswa"
              :urlbackend="'/system/usersmahasiswa/' + user_id"
              :datauser="formdata"
            />
          </b-col>
        </b-row>
...
<script>
  ...
  export default {
    ...
    methods: {
      async initialize() {
        var url = '/system/usersmahasiswa/' + this.user_id
        await this.$ajax
          .get(url, {
            headers: {
